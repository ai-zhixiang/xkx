#!/usr/bin/env python3
"""Patch bot_gateway.py: add mock payment + QR code flow"""
import re
...
old = '''    # 订阅检查：已绑账号直接走 Hermes，否则引导绑定手机
...
new = '''    # 订阅检查：已绑账号 → 检查订阅，未绑定 → 引导绑定手机
    ...
    if backend != "deepseek" and user_account_id:
        import re as _re2
        stripped = _re2.sub(r'[,。!?、;:\"\'\\s]', '', content).lower()
        if stripped in ("开通", "mock", "开通会员"):
            # Mock 支付 → 生成 Bot 二维码
            try:
                ...
                async with _asf() as _s:
                    ...
                    await _s.execute(
                        ...
                        {"oid": openid or user_id, "exp": datetime.date.today() + datetime.timedelta(days=30),

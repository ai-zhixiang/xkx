...
@app.post('/api/bot/push-write-song', response_class=JSONResponse)
async def push_write_song(data: dict):
    """已绑定用户回访时，推一条回归消息到 Bot"""
    import asyncpg, httpx
    openid = data.get('openid', '')
    nickname = data.get('nickname', '') or '虾友'
    if not openid:
    ...
    try:
        ...
        if not row:
            return {"ok": False, "error": "未找到绑定记录"}
        cuid = row["channel_user_id"]
        # channel_bindings 存的 user_id 不带后缀，bot_accounts 带 @im.wechat
        br = await conn.fetchrow(
            "SELECT bot_id FROM bot_accounts WHERE (user_id=$1 OR user_id LIKE $2) AND is_active=true LIMIT 1",
            cuid, cuid + "@%"
        )
        if not br:
            return {"ok": False, "error": "未找到 Bot"}
        bot_id = br["bot_id"]
        # 推消息时需要加 @im.wechat 后缀
        to_user = cuid + "@im.wechat" if "@" not in cuid else cuid
        async with httpx.AsyncClient(timeout=5) as hc:
            ...
            if kr.status_code == 200:
                ...
                return {"ok": False, "error": f"push status={kr.status_code}"}
    finally:
        await conn.close()

@app.get('/player/promo-haica', response_class=HTMLResponse)
...
async def xiakexing_page():
    with open("app/templates/xiakexing.html", "r", encoding="utf-8") as f:
        return HTMLResponse(f.read())


@app.get('/api/wechat/jsconfig')
async def wechat_jsconfig(url: str = ''):
    import time, random, string, hashlib, httpx
    appid = os.getenv('WECHAT_APPID', 'wx79c1f7db6d290510')
    secret = os.getenv('WECHAT_APPSECRET', '')
    if not secret:
        return JSONResponse({'errcode': -1, 'errmsg': 'no secret'})
    try:
        async with httpx.AsyncClient(timeout=10) as c:
            r = await c.get('https://api.weixin.qq.com/cgi-bin/token', params={
                'grant_type': 'client_credential', 'appid': appid, 'secret': secret
            })
            token_data = r.json()
            access_token = token_data.get('access_token', '')
            if not access_token:
                return JSONResponse({'errcode': -1, 'errmsg': 'token failed'})
            r2 = await c.get('https://api.weixin.qq.com/cgi-bin/ticket/getticket', params={
                'access_token': access_token, 'type': 'jsapi'
            })
            ticket_data = r2.json()
            ticket = ticket_data.get('ticket', '')
            if not ticket:
                return JSONResponse({'errcode': -1, 'errmsg': 'ticket failed'})
    except Exception as e:
        return JSONResponse({'errcode': -1, 'errmsg': str(e)})
    nonce = ''.join(random.choices(string.ascii_lowercase + string.digits, k=16))
    ts = int(time.time())
    sign_str = f'jsapi_ticket={ticket}&noncestr={nonce}&timestamp={ts}&url={url}'
    signature = hashlib.sha1(sign_str.encode()).hexdigest()
    return {'appId': appid, 'timestamp': ts, 'nonceStr': nonce, 'signature': signature}
